...
The public/API projection does not expose a general-purpose current-place prose field. Mapped identity keys remain a narrow `currentPlaceDetails` surface. Stronger facts use discriminated `currentPlaceClaims`: canonical website URL, structured opening-hours schedule, finite access/fee/wheelchair values, or integer capacity. Operator and description prose remain internal because a single approval cannot bound the unrelated claims embedded in arbitrary text. An editorial recommendation emits only the fixed recommendation component. Arbitrary source notes, review notes, publication notes and claim-evidence notes are internal and are never copied to the public package. Non-editorial claim-constrained descriptions are replaced by the standard mapped-context wording; editorial copy requires an editorial profile and current Tier E evidence.

Project research notes, source limitation and coverage notes, map notes, town-study working notes, imported curation metadata, and free-text date wording are also internal. The public package retains the structured source identity, canonically validated HTTP(S) URL, access date, reliability, and licence needed for provenance. Geometry, additional points, boundaries, and settlement polygons are reconstructed field by field; coordinate arrays must be dense and recursive collections have a maximum nesting depth of 32. CSV exports are generated from this same public representation and every textual cell passes the shared control/formula guard. Source packages are not mutated, so the full research history remains available to curators.
