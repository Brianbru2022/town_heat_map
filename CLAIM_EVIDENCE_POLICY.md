...
Project research notes, source limitation and coverage notes, map notes, town-study working notes, imported curation metadata, and free-text date wording are also internal. The public package retains the structured source identity, safe HTTP(S) URL, access date, reliability, and licence needed for provenance. Geometry, additional points, boundaries, and settlement polygons are reconstructed field by field. CSV exports are generated from this same public representation rather than copying source-file columns. Source packages are not mutated, so the full research history remains available to curators.
